...
ftp::replycodea
ftp::ftpbuf::ftpdata(int portno, istream* i, ostream* o, const char* cmd, const char* arg) {
...
ftp::replycodea
ftp::ftpbuf::send_cmd(const char* cmd, const char* arg) {
...
void
ftp::ftpbuf::serve_clients(int portno)
	// right now no server ftp class can be used as a server
{
}

ftp::replycodea
ftp::ftpbuf::cd(const char* dir) {
	return send_cmd("CWD", dir);
}

ftp::replycodea
ftp::ftpbuf::useraddr(sockinetaddr sa) {
	if (sa.sin_addr.s_addr == 0) {
		// local host
		char	hostname[64];
		if (::gethostname(hostname, 63) == -1)
			throw sockerr(EADDRNOTAVAIL);
...
ftp::replycodea
ftp::ftpbuf::useraddr(const char* hostname, int portno) {
	return useraddr(sockinetaddr(hostname, portno));
}

ftp::replycodea
ftp::ftpbuf::server_port(int portno) {
	int		hi_portno	= portno >> 8;
	int		lo_portno	= portno & 0xff;
	char	port[80];
...
ftp::replycodea
ftp::ftpbuf::rep_type(ftp::reptype rt) {
	return send_cmd("TYPE", ::reptype[int(rt)]);
}

ftp::replycodea
ftp::ftpbuf::file_stru(ftp::filestru fs) {
	return send_cmd("STRU", ::filestru[int(fs)]);
}

ftp::replycodea
ftp::ftpbuf::trans_mode(ftp::transmode tm) {
	return send_cmd("STRU", ::transmode[int(tm)]);
}

ftp::replycodea
ftp::ftpbuf::getfile(const char* rpath, const char* lpath) {
	if (lpath == 0) {
		lpath = rpath;
	}
...
ftp::replycodea
ftp::ftpbuf::list(const char* rpath, int justnames) {
	if (justnames) {
...
ftp::replycodea
ftp::ftpbuf::putfile(const char* lpath, const char* rpath) {
	if (rpath == 0) {
		rpath = lpath;
	}
...
ftp::replycodea
ftp::ftpbuf::putfile(const char* lpath) {
	if (lpath == 0) {
		return ftp::rca_error;
	}
...
ftp::replycodea
ftp::ftpbuf::append(const char* lpath, const char* rpath) {
	if (lpath == 0) {
		return ftp::rca_error;
	}
...
ftp::replycodea
ftp::ftpbuf::allocate(int numbytes) {
	char	b[32];
...
ftp::replycodea
ftp::ftpbuf::rename(const char* rpath, const char* newrpath) {
...
ftp::replycodea
ftp::ftpbuf::rmfile(const char* rpath) {
	return send_cmd("DELE", rpath);
}

ftp::replycodea
ftp::ftpbuf::rmdir(const char* rpath) {
	return send_cmd("RMD", rpath);
}

ftp::replycodea
ftp::ftpbuf::mkdir(const char* rpath) {
