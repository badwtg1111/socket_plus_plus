...
void
smtp::smtpbuf::send_cmd(const char* cmd, const char* s, const char* p) {
...
void
smtp::smtpbuf::mail(const char* reverse_path) {
	if (reverse_path) {
...
void
smtp::smtpbuf::rcpt(const char* forward_path) {
	if (forward_path) {
...
void
smtp::smtpbuf::help(const char* s) {
	send_cmd("HELP ", s);
}

void
smtp::smtpbuf::send_buf(const char* buf, int len) {
	if (buf == 0 || len <= 0) {
		return;
	}
...
void
smtp::smtpbuf::data(const char* buf, int len) {
	data();
	send_buf(buf, len);
	xsputn("\r\n.\r\n", 5);
	sync();
...
void
smtp::smtpbuf::data(const char* filename) {
	data();
...
int
smtp::get_response(char* buf, int len)
...
void
smtp::smtpbuf::serve_clients(int portno) {
}
