...
void
Fork::ForkProcess::kill_process() const {
	if (pid > 0) {
		kill(pid, SIGKILL);
		reap_child();
	}
...
void
Fork::ForkProcess::reap_child() const {
	int	status;
...
void
Fork::ForkProcess::infanticide_reason(pid_t pid, int status) {
	if (pid <= 0) {
		return;
	}

	if (WIFSTOPPED(status)) {
		cerr << "process " << pid << " gets " << SYS_SIGLIST[WSTOPSIG(status)] << endl;
	} else if (WIFEXITED(status)) {
		cerr << "process " << pid << " exited with status " << WEXITSTATUS(status) << endl;
	} else if (WIFSIGNALED(status)) {
		cerr << "process " << pid << " got " << SYS_SIGLIST[WTERMSIG(status)] << endl;
	}
}

void
Fork::ForkProcess::reaper_nohang(int signo) {
	if (signo != SIGCHLD) {
		return;
	}
...
void
Fork::ForkProcess::commit_suicide(int) {
