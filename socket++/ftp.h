...
public:
	...
	class ftpbuf : public protocol::protocolbuf {
		...
		replycodea		ftpdata(int portno, std::istream* i, std::ostream* out, const char* cmd, const char* arg = 0);
	...
	public:
		...
		replycodea
		help() {
			return send_cmd("HELP");
		}
		replycodea
		noop() {
			return send_cmd("NOOP");
		}
		replycodea
		quit() {
		...
		replycodea
		system() {
			return send_cmd("SYST");
		}
		replycodea
		status() {
			return send_cmd("STAT");
		}
