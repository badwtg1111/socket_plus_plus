...
bool
sig::unset(int signo, sig::hnd* hnd) {
	if (hnd == 0) {
		return false;
	}
...
void
sig::mask(int signo) const {
	sigset_t	s;
	if (sigemptyset(&s) == -1) {
		throw sigerr();
	}
	if (sigaddset(&s, signo) == -1) {
		throw sigerr();
	}

	if (sigprocmask(SIG_BLOCK, &s, 0) == -1) {
		throw sigerr();
	}
}

void
sig::unmask(int signo) const {
	sigset_t	s;
	if (sigemptyset(&s) == -1) {
		throw sigerr();
	}
	...
	if (sigprocmask(SIG_UNBLOCK, &s, 0) == -1) {
		throw sigerr();
	}
}

void
sig::mask(int siga, int sigb) const {
	struct sigaction	sa;
	if (sigaction(siga, 0, &sa) == -1) {
		throw sigerr();
	}
	if (sa.sa_handler != sighnd_type(&sighandler)) {
		sa.sa_handler = (void(*) (int)) sighnd_type(&sighandler);
	...
	if (sigaddset(&sa.sa_mask, sigb) == -1) {
		throw sigerr();
	}
	if (sigaction(siga, &sa, 0) == -1) {
		throw sigerr();
	}
}

void
sig::unmask(int siga, int sigb) const {
	struct sigaction	sa;
	if (sigaction(siga, 0, &sa) == -1) {
		throw sigerr();
	}
	if (sa.sa_handler != sighnd_type(&sighandler)) {
		...
		if (sigdelset(&sa.sa_mask, sigb) == -1) {
			throw sigerr();
		}
	}
	if (sigaction(siga, &sa, 0) == -1) {
...
void
sig::sysresume(int signo, bool set) const {
	struct sigaction	sa;
	if (sigaction(signo, 0, &sa) == -1) {
		throw sigerr();
	}
